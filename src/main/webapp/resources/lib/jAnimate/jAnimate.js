		...
		(function(a) {
			function b(b) {
				var c = this;
						...
						this.next = function() {
							a(this.el)
									.jAnimateOnce(
											this.effects[this.indx],
											function() {
														c.indx++,
														"undefined" != typeof c.effects[c.indx] ? c
																.next()
			...
			function c(b) {
				var c = this;
						...
						this.older = a(this.opt.el).attr("data-janimate"),
						this.newer = d["class"] + this.opt.effect,
						this.once = this.opt.once,
						a(this.opt.el).removeClass(this.older).removeAttr(
								d.data),
						this.opt.el.offsetWidth = this.opt.el.offsetWidth,
						a(this.opt.el).addClass(this.newer).attr(d.data,
								this.newer),
						a(this.opt.el)
								.one(
										d.animationend,
										function() {
													c.opt.once
															&& a(c.opt.el)
																	...
																	.removeAttr(
																			d.data),
													"undefined" != typeof c.opt.callback
															&& a
																	.isFunction(c.opt.callback)
															&& c.opt
																	.callback(
																			c,
																			c.opt.effect)
